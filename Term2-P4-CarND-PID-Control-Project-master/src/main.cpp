...
int main(){
  ...
  // Steering gains
  double Kp_steer = 0.270;
  double Ki_steer = 0.0003;
  double Kd_steer = 3.0;
  pid_steer.Init(Kp_steer, Ki_steer, Kd_steer);


  // Throttle gains
  double Kp_throttle = 0.32;
  double Ki_throttle = 0.0;
  double Kd_throttle = 0.023; 
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2'){
      ...
      if (s != "") {
        ...
        std::string event = j[0].get<std::string>();
          ...
          // Run PID on cte to produce a steering value
          pid_steer.current_sample_number = pid_steer.current_sample_number+ 1;
